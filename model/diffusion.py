...
class Diffusion(BaseModule):
    def __init__(self, n_feats, dim,
                 ...
                 beta_min=0.05, beta_max=20, pe_scale=1000,  a =0.5, b=0.5, c=0.5, d = 1, n_timesteps = 10):
    ...
    def loss_t(self, X0, mask, mu, n, spk=None):
        Xn, Xns = self.forward_diffusion(X0, mask, mu, n)
        time = n.unsqueeze(-1).unsqueeze(-1)
        noise_estimation = self.estimator(Xn, mask, mu, n, spk)
        loss = torch.sum((noise_estimation - Xns)**2) / (torch.sum(mask)*self.n_feats)
        return loss, Xn
